...
const ProductDetailPresentations = ({ product, presentaciones, onPresentacionesChange, loading, error }) => {
  ...
  const openDeletePopover = () => {
    ...
    const anchor = delBtnRef.current?.getDomRef?.() || delBtnRef.current;
  ...
  const renderFilesByType = () => {
    ...
    return (
      <FlexBox direction="Column" style={{ gap: '1.5rem' }}>
            ...
            <FlexBox direction="Column" style={{ gap: '0.5rem' }}>
              ...
              <Carousel style={{ width: '100%', backgroundColor: '#fafafa', borderRadius: '8px', border: '1px solid #e0e0e0', minHeight: '140px' }}>
                {imageFiles.map((file, idx) => (
                  <FlexBox key={file.FILEID || idx} alignItems="Center" justifyContent="Center" style={{ height: '200px', padding: '0.5rem', maxWidth: '300px', maxHeight: '300px', overflow: 'hidden' }}>
                    <img src={file.FILE} alt={file.INFOAD || `Imagen ${idx + 1}`} style={{ width: '100%', height: '100%', maxWidth: '300px', maxHeight: '300px', objectFit: 'contain', borderRadius: '8px', border: '2px solid #fff', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', display: 'block' }} />
                  </FlexBox>
                ))}
              </Carousel>
            </FlexBox>
        ...
        {pdfFiles.length > 0 && (
          <div>
            <Label style={{ fontSize: '0.75rem', color: '#666', marginBottom: '0.5rem', display: 'block' }}><Icon name="pdf-attachment" style={{ marginRight: '0.25rem', fontSize: '0.875rem' }} />PDFs ({pdfFiles.length})</Label>
            <FlexBox style={{ gap: '0.5rem', flexWrap: 'wrap' }}>
              {pdfFiles.map((file, idx) => (
                <FlexBox key={file.FILEID || idx} direction="Column" alignItems="Center" style={{ padding: '0.5rem', backgroundColor: '#fff3f3', borderRadius: '4px', border: '1px solid #ffcdd2', cursor: 'pointer', width: '80px', transition: 'box-shadow 0.2s', boxShadow: '0 2px 6px rgba(255,205,210,0.15)' }} onClick={() => window.open(file.FILE, '_blank')} title={file.INFOAD || `PDF ${idx + 1}`}>
                  <Icon name="pdf-attachment" style={{ fontSize: '2rem', color: '#d32f2f', marginBottom: '0.25rem' }} />
                  <Text style={{ fontSize: '0.8rem', color: '#d32f2f', textAlign: 'center', wordBreak: 'break-word' }}>{file.INFOAD || `PDF ${idx + 1}`}</Text>
                </FlexBox>
              ))}
            </FlexBox>
          </div>
        ...
        {docFiles.length > 0 && (
          <Card style={{ boxShadow: 'none', background: 'transparent', padding: 0 }}>
            ...
            <FlexBox style={{ gap: '0.75rem', flexWrap: 'wrap' }}>
              {docFiles.map((file, idx) => (<Button key={file.FILEID || idx} icon="document" design="Default" style={{ minWidth: '100px', marginBottom: '0.5rem' }} onClick={() => window.open(file.FILE, '_blank')}>{file.INFOAD || `Doc ${idx + 1}`}</Button>))}
            </FlexBox>
          </Card>
        )}
        {videoFiles.length > 0 && (
        ...
        {otherFiles.length > 0 && (
          <Card style={{ boxShadow: 'none', background: 'transparent', padding: 0 }}>
            ...
            <FlexBox direction="Column" style={{ gap: '0.5rem' }}>
              {otherFiles.map((file, idx) => (<Button key={file.FILEID || idx} icon="download" design="Transparent" style={{ minWidth: '120px', marginBottom: '0.5rem' }} onClick={() => window.open(file.FILE, '_blank')}>{file.INFOAD || 'Archivo sin nombre'}</Button>))}
            </FlexBox>
          </Card>
        )}
      </FlexBox>
  ...
  return (
    <div style={{ padding: '1.5rem', overflowY: 'auto', height: '100%' }}>
        ...
        <FlexBox direction="Column" style={{ gap: '1rem' }}>
          <FlexBox alignItems="Center" justifyContent="SpaceBetween">
            ...
            <FlexBox alignItems="Center" style={{ gap: '0.5rem' }}>
              {selectedPresenta && <PresentationStatus presentation={selectedPresenta} onStatusChange={handleStatusChange} />}
          ...
          {!loading && !error && presentaciones.length > 0 && (
            <Select value={selectedPresenta?.IdPresentaOK || ''} onChange={handlePresentaChange} style={{ width: '100%' }}>
        ...
        {selectedPresenta ? (
          <FlexBox direction="Column" style={{ gap: '1.5rem', borderTop: '1px solid #e0e0e0', paddingTop: '1.5rem' }}>
            <FlexBox direction="Column" style={{ gap: '1rem' }}>
              ...
              {!loadingFiles && files.length === 0 && <Text style={{ color: '#666' }}>No hay archivos para esta presentación.</Text>}
              {!loadingFiles && files.length > 0 && renderFilesByType()}
            </FlexBox>

            <PresentationPriceViewer skuid={selectedPresenta.SKUID} idPresentaOK={selectedPresenta.IdPresentaOK} />
          </FlexBox>
        ) : (
          !loading && (
            <IllustratedMessage name="NoData" titleText="Sin Presentaciones" subtitleText="Este producto no tiene presentaciones o no se ha seleccionado ninguna." />
          )
        )}
      </FlexBox>
      ...
      <ResponsivePopover ref={delPopoverRef} placementType="Bottom">
        ...
        <Bar endContent={<><Button design="Transparent" onClick={() => delPopoverRef.current?.close()} disabled={deleting}>Cancelar</Button><Button design="Negative" icon="delete" onClick={handleDeleteSingle} disabled={deleting}>Eliminar</Button></>} />
      </ResponsivePopover>
    </div>
  );
};
